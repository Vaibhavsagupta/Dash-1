...
export default function ManageData() {
    ...
    const fetchStudents = async () => {
        try {
            let token = localStorage.getItem('access_token') || 'demo_teacher_token';
    ...
    const handleGoogleSync = async () => {
        if (!confirm('This will trigger the configured Python script to pull data from Google Sheets. Continue?')) return;
        try {
            const token = localStorage.getItem('access_token');
            const res = await fetch(`${API_BASE_URL}/ingest/trigger-sheet-sync`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            if (res.ok) {
                const data = await res.json();
                alert(`Sync Complete: ${data.message}. Updated ${data.records_updated || 0} records.`);
                fetchStudents();
            } else {
                alert('Sync failed.');
            }
    ...
    const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        reader.onload = async (event) => {
            ...
            // Now supports fees_paid, certifications
            ...
            for (let i = startIndex; i < lines.length; i++) {
                ...
                if (parts.length >= 7) {
                    // Check for extra columns if they exist
                    const fees = parts.length > 7 ? (parts[7].toLowerCase() === 'true' || parts[7] === '1') : undefined;
                    const certs = parts.length > 8 ? parseInt(parts[8]) : undefined;

                    // We only send updates for standard scores here as the bulk endpoint expects that schema.
                    // For flexible updates we would need a new endpoint.
                    // Let's stick to the existing schema for now or update it.
                    // For this task, we will just use the standard fields + fees if we update the backend router.
            ...
            // ... (rest of logic same)
            // Ideally we should use the new /ingest/csv which is more flexible, but for now let's keep the existing implementation
            // or switch to /ingest/csv. switching to /ingest/csv is better.

            // LET'S SWITCH to the new endpoint for CSV uploads as it is smarter
